...
\begin{itemize}
    ...
    \item \textbf{sagt für jeden Testcase einzeln aus, ob er ausgeführt wurde oder nicht}\\
    ...
    Es ist kaum realistisch anzunehmen, dass ein strukturiertes Format existiert, welches von jedem existierenden Testframework gleichermaßen benutzt wird. Dennoch gibt es durchaus gängige Formate, die von unterschiedlichen Testframeworks aus unterschiedlichen Sprachen unterstützt werden, wie im Folgenden in \chapref{Vergleich Testausgabefomate} untersucht wird. 
\end{itemize}

\section{Reproduzierbare Builds}\label{chap:determinism}
...
Ein vollständige Garantie des Reproduziebarkeit zwischen Systemen ist nur schwer zu erreichen, jedoch bietet \texttt{Nix} in diesem Fall eine ausreichende Grundlage. 
...
Die Form eines Testausgabeformates hängt im Wesentlichen von den folgenden Faktoren ab, deren Aufteilung der von \textcite{winkler} vorgestellten Struktur eines Testframeworks folgt: % TODO test discovery hinzufügen?
\begin{itemize}
  ...
  \newpage
  \item \textbf{Struktur der zugrunde liegenden Testdateien:}\\
...
\begin{lstlisting}[caption={direkter Vergleich von JUnit XML, erzeugt durch JUnit5 mit Jupiter in Java. Jeder Ausgabe liegen dieselben beiden Testdateien zugrunde.\\ ''\textellipsis{}'' bezeichnet gekürzte Stellen.}, label={JAVACompare}]
...
So ignoriert Gradle beispielsweise in diesem exemplarischen Fall das Element \texttt{<properties>}, während Maven und Jupiter (ohne Build-Tool) eine exzessive Liste von \texttt{<property>} in der Testausgabe mitgeben. \\
...
Ähnliches bei der Benennung der Testsuite: Da Gradle und Maven für jede Datei eine eigene Ausgabedatei erzeugen, benutzen die beiden Build-Tool den Namen der jeweiligen Testsuite, hier ''BasicTest'', als Wert des Attributs \texttt{name} im Element \texttt{<testsuite>}. Jupiter gibt dagegen nur eine Ausgabedatei aus, die das Ergebnis aller Testdateien enthält, und benutzt in dieser den Standardwert ''Junit Jupiter'' als Namen für \texttt{<testsuite>}.
...
Dieser Sachverhalt muss in \chapref{Vergleich Testausgabefomate} bei der Bewertung der Struktur der einzelnen Testausgabeformate berücksichtigt werden. 
